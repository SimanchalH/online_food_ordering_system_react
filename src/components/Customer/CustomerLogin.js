...
const Login = ({ login, isAuthenticated }) => {
  ...
  const onSubmit = async (e) => {
    ...
    axios({
      method: 'post',
      url: `${config.api_url}/customer-login`,
      data: formData,
    })
      .then(function (response) {
        ...
        if (response.data.customer_id) {
          ...
          window.sessionStorage.setItem("user_id", response.data.customer_id);
          window.sessionStorage.setItem("user_level_id", "2");
          window.sessionStorage.setItem("user_name", response.data.customer_first_name+" "+response.data.customer_last_name);
          navigate("/dashboard")
        } else {
          ...
          setMessage({
            ...
            msg: 'Invalid Customername or Password. Kindly try again !!!!'
          });
        }
      })
        ...
        <div className="container">
          <div className="row">
            <div className="col-lg-12">
              <h2 className="pageTitle">Customer Login</h2>
            </div>
          </div>
        </div>
      </section>
